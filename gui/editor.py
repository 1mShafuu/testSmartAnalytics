...
class TableEditorApp(ctk.CTk):
    ...
    def add_new_table(self):
        """Функция для добавления новой таблицы"""
        for widget in self.fields_frame.winfo_children():
            widget.destroy()
    ...
    def save_new_table(self):
        ...
        try:
            ...
            # Очищаем форму после создания таблицы
            for widget in self.fields_frame.winfo_children():
                widget.destroy()
            self.field_entries.clear()
    ...
    def add_new_field(self):
        ...
        # Проверяем существующие поля только в текущей форме
        existing_field_names = {entry[0].get() for entry in self.field_entries}

        # Генерация уникального имени для нового поля
    ...
    def show_save_cancel_buttons(self):
        """Отображает кнопки для сохранения и отмены изменений"""
...
#####
    ...
    def save_changes(self):
        ...
        try:
            ...
            # Удаляем поля, которых больше нет в новом наборе
            for field_name in current_field_names:
                if field_name not in new_field_names:
                    self.db_manager.delete_column(selected_table, field_name)

            # Обрабатываем изменения типов данных и добавляем новые столбцы
            for field_entry in self.field_entries:
                field_name = field_entry[0].get()
                ...
                if field_name not in current_field_names:
                    ...
                    # Изменение типа столбца, если он изменился
            ...
            # Добавляем новый первичный ключ, если он был выбран
            for field_entry in self.field_entries:
                if field_entry[2].get():  # если поле отмечено как первичный ключ
                    self.db_manager.add_primary_key(
                        table_name=selected_table,
                        column_name=field_entry[0].get()
                    )
                    break  # так как у нас может быть только один первичный ключ

            self.show_info_message(f"Изменения в таблице '{selected_table}' сохранены.")
            self.show_table_form(selected_table)  # Обновляем форму
        ...
        if response:
            try:
                self.db_manager.alter_column_type_with_using(
                    ...
                    new_type=new_type.rstrip('",.')  # Передаем строковое значение типа напрямую
                )
    ...
    def cancel_changes(self):
        """Отменяет изменения и перезагружает форму"""
        selected_table = self.table_radio_var.get()
        if selected_table:
            try:
